...
def convert_gas_onefile(
    ...
    filenumber = int(f.filename.split('.')[1])
    ...
    plt.tight_layout()
    plt.savefig(f'dust_to_gas_ratio_{filenumber:02d}.png', dpi=300)
    ...
    ax.set_ylabel(r'$M_{\rm gas}$')
    plt.tight_layout()
    plt.savefig(f'metallicity_total_{filenumber:02d}.png', dpi=300)
    plt.close()
    ...
    ax.set_ylabel(r'$M_{\rm gas}$')
    plt.tight_layout()
    plt.savefig(f'metallicity_Oxygen_{filenumber:02d}.png', dpi=300)
    plt.close()
    ...
    ax.set_xscale('log')
    plt.tight_layout()
    plt.savefig(f'dust_to_metal_ratio_{filenumber:02d}.png', dpi=300)
    plt.close()

    fig, ax = plt.subplots(figsize=default_figsize)
    h, xedges, yedges = np.histogram2d(metallicity[mask], logOH_12[mask], bins=50,
                                      weights=mass.to('M_sun').value[mask])
    X, Y = np.meshgrid(xedges[:-1], yedges[:-1])
    im = ax.pcolormesh(xedges, yedges, h.T, cmap='viridis')
    fig.colorbar(im, ax=ax, label=r'Total gas mass ($M_{\odot}$)')
    #ax.set_xscale('log')
    #ax.set_yscale('log')
    ax.set_ylabel(r'$\log_{10}(n_{\rm O}/n_{\rm H}) + 12$')
    ax.set_xlabel(r'$Z$')
    ax.set_xlim(0.01, 0.03)
    ax.axvline(0.02, color='red', linestyle='--', label='Solar metallicity (old)')
    ax.axvline(0.014, color='blue', linestyle='--', label='Solar metallicity (new)')
    ax.axhline(8.69, color='green', linestyle='--', label='Solar Oxygen abundance')
    ax.legend()
    plt.tight_layout()
    plt.savefig(f'dust_to_gas_vs_metallicity_{filenumber:02d}.png', dpi=300)
    plt.close()
